...
void Graph::initReverse(){
    for(int i = 0;i<n;i++){
        for(int j = pstart[i]; j<pstart[i+1];j++){
            ...
            int reverse_edge_id = binary_search(edges, pstart[v], pstart[v+1], i);
...
//    }
    ...
    for(int i = 0;i<n;i++){
        for(int j = pstart[i]; j<pstart[i+1];j++){
//            if(neighbor_order_c2[j][0] != -1) continue;
            
            int v = edges[j];
            if(v<i) continue;
            neighbor_order_c2[reverse[j]].value = neighbor_order_c2[j].value = (num_c[j] + 4) / (2*(float)pow((long)((long)(degree[i]+1) * (long)(degree[v]+1)),0.5));
            neighbor_order_c2[j].vertex = v;
            neighbor_order_c2[reverse[j]].vertex = i;
            
//            if(isnan(neighbor_order_c[j][1])){
//                cout<<num_c[j]<<" "<<degree[i]+1<<" "<<degree[v]+1<<endl;
//            }
            
            neighbor_order_f2[reverse[j]].value = neighbor_order_f2[j].value = (num_f[j] + 12) / (6*(float)pow((long)((long)(degree[i]+1) * (long)(degree[v]+1)),0.5));
            
//            if(isnan(neighbor_order_f[j][1])){
//                cout<<num_f[j]<<" "<<degree[i]+1<<" "<<degree[v]+1<<endl;
//            }
